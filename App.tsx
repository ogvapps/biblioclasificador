...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleSettingsChange = () => {
      setAppName(localStorage.getItem('biblio_app_name') || 'BiblioClasificador');
      setCenterName(localStorage.getItem('biblio_center_name') || 'Centro Educativo');
      setCenterType(localStorage.getItem('biblio_center_type') || 'SCHOOL');
      setLogoUrl(localStorage.getItem('biblio_logo_url') || '');

      const primary = localStorage.getItem('biblio_primary_color');
      if (primary) {
        document.documentElement.style.setProperty('--color-primary', primary);
      }
    };

    // Call once on mount to set styles
    handleSettingsChange();

    window.addEventListener('biblio_settings_changed', handleSettingsChange);
    return () => window.removeEventListener('biblio_settings_changed', handleSettingsChange);
  }, [appName]);
  ...
  const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newBooks = importedBooks.filter((impBook: any) => {
        return !books.some(b =>
      ...
      if (newBooks.length > 0) {
        const booksToSave = newBooks.map(({ id, ...rest }: any) => rest as Omit<Book, 'id'>);
  ...
  const handleStudentFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Filter existing
      const newStudents = importedStudents.filter((impStu: any) => {
        return !registeredStudents.some(s => s.name.toLowerCase() === impStu.name?.toLowerCase());
      });

      if (newStudents.length > 0) {
        const studentsToSave = newStudents.map(({ id, ...rest }: any) => rest as Omit<Student, 'id'>);
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm safe-top">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            {logoUrl ? (
              <img src={logoUrl} alt="Logo" className="h-8 w-8 sm:h-10 sm:w-10 object-contain rounded-lg shadow-sm bg-white" />
            ) : (
            ...
            <div>
              <h1 className="text-base sm:text-lg md:text-xl font-bold text-slate-800 tracking-tight flex flex-col leading-tight">
                <span>{appName}</span>
                {centerName && <span className="text-[10px] sm:text-xs text-slate-500 font-medium">{centerName}</span>}
              </h1>
            </div>
              ...
              {userRole === 'STUDENT' ? (
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    title="Cerrar Sesión (Volver a Modo Público)"
                  >
        ...
        {userRole !== 'STUDENT' && (
          <div className="flex gap-4 mb-6 border-b border-slate-200 overflow-x-auto no-scrollbar mask-gradient-right">
            ...
            <button onClick={() => setCurrentTab('students')} className={`pb-3 px-2 text-sm font-bold flex items-center gap-2 border-b-2 whitespace-nowrap transition-colors ${currentTab === 'students' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500'}`}>
              <Users className="w-4 h-4" /> LECTORES
            </button>
